...
void ImageProcessingHelper::detect_subjects(std::vector<Image> &imageSubjects, const Image &background, int tolerance,
                                            int min_size_cc) {
	...
	for (int i = 0; i < imageSubjects.size(); ++i) {
		...
		ImageProcessingHelper::filter_cc(imageSubjects[i], min_size_cc);
	}
}
		...
		for (int y = 0; y < width; y++) {
			for (int k = 0; k < size; k++) {
				...
				if (subjectPixel != ImageProcessingHelper::pink && subjectPixel != ImageProcessingHelper::pinkRGBA) {
					...
					g = int((g1 * g2) / 255);
					b = int((b1 * b2) / 255);
					fadePixel.push_back(r);
					fadePixel.push_back(g);
					fadePixel.push_back(b);
...
Image ImageProcessingHelper::crop(const Image &img, int width, int height) {
